...
public class BarcodePDF417
{
    ...
    {
        int codePtr = 0;
        bitColumns = 17 * (codeColumns + 3) + 18;
    ...
    protected void calculateErrorCorrection(int dest)
    {
        ...
        for(int k = 0; k < lenCodewords; k++)
        {
            ...
            for(int e = 0; e <= lastE; e++)
            {
                ...
                int t3 = 929 - t2;
                codewords[dest + e] = ((e != lastE ? codewords[dest + e + 1] : 0) + t3) % 929;
            }

        }

        for(int k = 0; k < Alength; k++)
            codewords[dest + k] = (929 - codewords[dest + k]) % 929;
    ...
    private void textCompaction(byte input[], int start, int length)
    {
        ...
        for(int k = start; k < length; k++)
        {
            ...
            if((v & 0x100000) != 0)
            {
                ...
                {
                    dest[ptr++] = 29;
                    mode = (mode & 0x80000) == 0 ? mode : 0x10000;
                }
                dest[ptr++] = 913;
                dest[ptr++] = v & 0xff;
                fullBytes += 2;
    ...
    void breakString()
    {
        ...
        for(int k = 0; k < textLength; k++)
        {
            ...
            if(nd >= 13)
            {
                if(lastP != startN)
                {
                    ...
                    boolean lastTxt = c >= ' ' && c < '\177' || c == '\r' || c == '\n' || c == '\t';
                    for(int j = lastP; j < startN; j++)
                    {
                        c = (char)(text[j] & 0xff);
                        boolean txt = c >= ' ' && c < '\177' || c == '\r' || c == '\n' || c == '\t';
                        if(txt != lastTxt)
                        {
        ...
        for(int k = 0; k < segmentList.size(); k++)
        {
            ...
            if(checkSegmentType(v, 'B') && getSegmentLength(v) == 1 && checkSegmentType(vp, 'T') && checkSegmentType(vn, 'T') && getSegmentLength(vp) + getSegmentLength(vn) >= 3)
            {
                vp.end = vn.end;
    ...
    protected void assemble()
    {
        ...
        for(int k = 0; k < segmentList.size(); k++)
        {
            ...
            case 66: // 'B'
                codewords[cwPtr++] = getSegmentLength(v) % 6 == 0 ? 924 : 901;
    ...
    {
        int level = 8;
        for(int size = 512; level > 0; size >>= 1)
        {
            if(remain >= size)
                return level;
            level--;
    ...
    public void paintCode()
    {
        ...
        int maxErr = maxPossibleErrorLevel(928 - lenCodewords);
        if((options & 0x10) == 0)
            if(lenCodewords < 41)
                errorLevel = 2;
            else
        ...
        boolean fixedColumn = (options & 4) == 0;
        boolean skipRowColAdjust = false;
        int tot = lenCodewords + lenErr;
        if((options & 1) != 0)
        {
        ...
        if((options & 6) == 0)
        {
            ...
            if(aspectRatio > 1000F)
                aspectRatio = 1000F;
            double b = 73F * aspectRatio - 4F;
            double c = (-b + Math.sqrt(b * b + (double)(68F * aspectRatio * (float)(lenCodewords + lenErr) * yHeight))) / (double)(34F * aspectRatio);
            codeColumns = (int)(c + 0.5D);
            if(codeColumns < 1)
                codeColumns = 1;
            else
            if(codeColumns > 30)
        ...
        throws BadElementException
    {
        ...
        return Image.getInstance(bitColumns, codeRows, false, 256, (options & 0x80) != 0 ? 1 : 0, g4, null);
    }

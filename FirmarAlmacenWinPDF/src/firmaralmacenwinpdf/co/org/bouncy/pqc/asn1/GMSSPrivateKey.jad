...
// Source File Name:   GMSSPrivateKey.java
...
public class GMSSPrivateKey extends ASN1Object
{
    ...
    public GMSSPrivateKey(int index[], byte currentSeed[][], byte nextNextSeed[][], byte currentAuthPath[][][], byte nextAuthPath[][][], Treehash currentTreehash[][], Treehash nextTreehash[][], 
    ...
    private ASN1Primitive encode(int index[], byte currentSeeds[][], byte nextNextSeeds[][], byte currentAuthPaths[][][], byte nextAuthPaths[][][], byte keep[][][], Treehash currentTreehash[][], 
            ...
            AlgorithmIdentifier algorithms[])
    {
        ...
        for(int i = 0; i < currentStack.length; i++)
        {
            for(int j = 0; j < currentStack[i].size(); j++)
                curStackPart0.add(new DEROctetString((byte[])(byte[])currentStack[i].elementAt(j)));
        ...
        for(int i = 0; i < nextStack.length; i++)
        {
            for(int j = 0; j < nextStack[i].size(); j++)
                nextStackPart0.add(new DEROctetString((byte[])(byte[])nextStack[i].elementAt(j)));
        ...
        for(int i = 0; i < currentRetain.length; i++)
        {
            for(int j = 0; j < currentRetain[i].length; j++)
            {
                for(int k = 0; k < currentRetain[i][j].size(); k++)
                    currentRetainPart0.add(new DEROctetString((byte[])(byte[])currentRetain[i][j].elementAt(k)));
        ...
        for(int i = 0; i < nextRetain.length; i++)
        {
            for(int j = 0; j < nextRetain[i].length; j++)
            {
                for(int k = 0; k < nextRetain[i][j].size(); k++)
                    nextRetainPart0.add(new DEROctetString((byte[])(byte[])nextRetain[i][j].elementAt(k)));
            ...
            if(nextNextRoot[i].getRetain() != null)
            {
                for(int j = 0; j < nextNextRoot[i].getRetain().length; j++)
                {
                    for(int k = 0; k < nextNextRoot[i].getRetain()[j].size(); k++)
                        currentRetainPart0.add(new DEROctetString((byte[])(byte[])nextNextRoot[i].getRetain()[j].elementAt(k)));
    ...
    private static int checkBigIntegerInIntRange(ASN1Encodable a)
    {
        ...
        if(b.compareTo(BigInteger.valueOf(0x7fffffffL)) > 0 || b.compareTo(BigInteger.valueOf(0xffffffff80000000L)) < 0)

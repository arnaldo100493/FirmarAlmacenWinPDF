...
// Source File Name:   IETFUtils.java
        ...
        while(nTok.hasMoreTokens()) 
        {
            ...
            if(token.indexOf('+') > 0)
            {
                ...
                if(pTok.hasMoreTokens())
                {
                    ...
                    for(; pTok.hasMoreTokens(); values.addElement(unescape(value)))
                    {
    ...
    public static String valueToString(ASN1Encodable value)
    {
        ...
        if((value instanceof ASN1String) && !(value instanceof DERUniversalString))
        {
            ...
            if(v.length() > 0 && v.charAt(0) == '#')
                vBuf.append((new StringBuilder()).append("\\").append(v).toString());
            else
                vBuf.append(v);
        } else
        {
            try
            {
                vBuf.append((new StringBuilder()).append("#").append(bytesToString(Hex.encode(value.toASN1Primitive().getEncoded("DER")))).toString());
            }
            catch(IOException e)
            {

...
// Source File Name:   DualECSP800DRBG.java
    ...
    implements SP80090DRBG
{
    ...
    public DualECSP800DRBG(Digest digest, int securityStrength, EntropySource entropySource, byte personalizationString[], byte nonce[])
    {
        ...
        if(entropySource.entropySize() < securityStrength || entropySource.entropySize() > 4096)
            throw new IllegalArgumentException((new StringBuilder()).append("EntropySource must provide between ").append(securityStrength).append(" and ").append(4096).append(" bits").toString());
        ...
        if(securityStrength <= 128)
        {
            ...
            _seedlen = 256;
            _outlen = 30;
            _curve = (co.org.bouncy.math.ec.ECCurve.Fp)NISTNamedCurves.getByName("P-256").getCurve();
            _P = new co.org.bouncy.math.ec.ECPoint.Fp(_curve, new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p256_Px), new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p256_Py));
            _Q = new co.org.bouncy.math.ec.ECPoint.Fp(_curve, new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p256_Qx), new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p256_Qy));
        } else
        if(securityStrength <= 192)
        {
            if(Utils.getMaxSecurityStrength(digest) < 192)
                throw new IllegalArgumentException("Requested security strength is not supported by digest");
            _seedlen = 384;
            _outlen = 46;
            _curve = (co.org.bouncy.math.ec.ECCurve.Fp)NISTNamedCurves.getByName("P-384").getCurve();
            _P = new co.org.bouncy.math.ec.ECPoint.Fp(_curve, new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p384_Px), new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p384_Py));
            _Q = new co.org.bouncy.math.ec.ECPoint.Fp(_curve, new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p384_Qx), new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p384_Qy));
        } else
        if(securityStrength <= 256)
        {
            ...
            _seedlen = 521;
            _outlen = 63;
            _curve = (co.org.bouncy.math.ec.ECCurve.Fp)NISTNamedCurves.getByName("P-521").getCurve();
            _P = new co.org.bouncy.math.ec.ECPoint.Fp(_curve, new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p521_Px), new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p521_Py));
            _Q = new co.org.bouncy.math.ec.ECPoint.Fp(_curve, new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p521_Qx), new co.org.bouncy.math.ec.ECFieldElement.Fp(_curve.getQ(), p521_Qy));
        } else
        {
    ...
    public int generate(byte output[], byte additionalInput[], boolean predictionResistant)
    {
        ...
        if(_reseedCounter + (long)m > 0x80000000L)
            return -1;
    ...
    private int _outlen;
    private co.org.bouncy.math.ec.ECCurve.Fp _curve;
    private ECPoint _P;
    private ECPoint _Q;
    private byte _s[];

...
// Source File Name:   JPAKEUtil.java
...
public class JPAKEUtil
{
        ...
        throws CryptoException
    {
        if(participantId1.equals(participantId2))
            throw new CryptoException((new StringBuilder()).append("Both participants are using the same participantId (").append(participantId1).append("). This is not allowed. ").append("Each participant must use a unique participantId.").toString());
        else
            return;
    }
        ...
        throws CryptoException
    {
        if(!expectedParticipantId.equals(actualParticipantId))
            throw new CryptoException((new StringBuilder()).append("Received payload from incorrect partner (").append(actualParticipantId).append("). Expected to receive payload from ").append(expectedParticipantId).append(".").toString());
        else
            return;
    }
    ...
    {
        if(object == null)
            throw new NullPointerException((new StringBuilder()).append(description).append(" must not be null").toString());
        else
            return;
    }
    ...
    public static void validateMacTag(String participantId, String partnerParticipantId, BigInteger gx1, BigInteger gx2, BigInteger gx3, BigInteger gx4, BigInteger keyingMaterial, Digest digest, 

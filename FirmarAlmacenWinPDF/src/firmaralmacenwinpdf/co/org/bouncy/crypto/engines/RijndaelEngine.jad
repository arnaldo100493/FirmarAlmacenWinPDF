...
// Source File Name:   RijndaelEngine.java
    ...
    implements BlockCipher
{
    ...
    private long applyS(long r, byte box[])
    {
        ...
        for(int j = 0; j < BC; j += 8)
            res |= (long)(box[(int)(r >> j & 255L)] & 0xff) << j;
    ...
    private void MixColumn()
    {
        ...
        for(int j = 0; j < BC; j += 8)
        {
            int a0 = (int)(A0 >> j & 255L);
            int a1 = (int)(A1 >> j & 255L);
            int a2 = (int)(A2 >> j & 255L);
            int a3 = (int)(A3 >> j & 255L);
    ...
    private void InvMixColumn()
    {
        ...
        for(int j = 0; j < BC; j += 8)
        {
            ...
            int a2 = (int)(A2 >> j & 255L);
            int a3 = (int)(A3 >> j & 255L);
            a0 = a0 == 0 ? -1 : logtable[a0 & 0xff] & 0xff;
            a1 = a1 == 0 ? -1 : logtable[a1 & 0xff] & 0xff;
            a2 = a2 == 0 ? -1 : logtable[a2 & 0xff] & 0xff;
            a3 = a3 == 0 ? -1 : logtable[a3 & 0xff] & 0xff;
    ...
    private long[][] generateWorkingKey(byte key[])
    {
        ...
        while(t < (ROUNDS + 1) * (BC / 8)) 
        {
            ...
            i = 0;
            while(i < KC && t < (ROUNDS + 1) * (BC / 8)) 
            {
                for(int i = 0; i < 4; i++)
                    W[t / (BC / 8)][i] |= (long)(tk[i][i] & 0xff) << (t * 8) % BC;
    ...
    public void init(boolean forEncryption, CipherParameters params)
    {
        ...
        {
            throw new IllegalArgumentException((new StringBuilder()).append("invalid parameter passed to Rijndael init - ").append(params.getClass().getName()).toString());
        }
    }

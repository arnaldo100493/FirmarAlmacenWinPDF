...
// Source File Name:   TwofishEngine.java
    ...
    implements BlockCipher
{
    ...
    public void init(boolean encrypting, CipherParameters params)
    {
        ...
        {
            throw new IllegalArgumentException((new StringBuilder()).append("invalid parameter passed to Twofish init - ").append(params.getClass().getName()).toString());
        }
    }
    ...
    private void setKey(byte key[])
    {
        ...
        for(int i = 0; i < 256; i++)
        {
            ...
            case 2: // '\002'
                ...
                gSBox[i * 2 + 1] = gMDS1[P[0][P[1][b1] & 0xff ^ b1(k1)] & 0xff ^ b1(k0)];
                gSBox[i * 2 + 512] = gMDS2[P[1][P[0][b2] & 0xff ^ b2(k1)] & 0xff ^ b2(k0)];
                gSBox[i * 2 + 513] = gMDS3[P[1][P[1][b3] & 0xff ^ b3(k1)] & 0xff ^ b3(k0)];
                break;
            }
        }
    ...
    {
        int b = x >>> 24 & 0xff;
        int g2 = (b << 1 ^ ((b & 0x80) == 0 ? 0 : 0x14d)) & 0xff;
        int g3 = b >>> 1 ^ ((b & 1) == 0 ? 0 : 0xa6) ^ g2;
        return x << 8 ^ g3 << 24 ^ g2 << 16 ^ g3 << 8 ^ b;
    }

    private int LFSR1(int x)
    {
        return x >> 1 ^ ((x & 1) == 0 ? 0 : 0xb4);
    }

    private int LFSR2(int x)
    {
        return x >> 2 ^ ((x & 2) == 0 ? 0 : 0xb4) ^ ((x & 1) == 0 ? 0 : 0x5a);
    }
